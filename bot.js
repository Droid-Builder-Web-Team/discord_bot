...
const Battle = require('./battle.js'); // User battles
const DogHouse = require('./doghouse.js'); // User discipline
...
client.on("message", async (message) => {
  ...
  if (DogHouse.getStatus(message.author.discriminator)) {
    DogHouse.disciplineUser(message.author.discriminator, message);
  ...
  if (commands.includes(parts[0])) {
    ...
    if (parts[0] === '!doghouseadd') {
      DogHouse.addUser(parts[1], message.author.discriminator);
    }

    if (parts[0] === '!doghouseremove') {
      DogHouse.removeUser(parts[1], message.author.discriminator);
    }

    if (parts[0] === '!doghouseodds') {
      DogHouse.updateOdds(parts[1], parts[2], message.author.discriminator);
    }    
  }
